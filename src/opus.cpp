...
int opusCodec::decodeFrame(istream &fin, ostream &fout)
{
    ...
    while (extractRTPPayload(fin, voipCodec::RD_buffer) > 0) {
        outSamples = opus_decode(decInst, (unsigned char *)RD_buffer->p.data, 
                RD_buffer->p.hdr.plen, (opus_int16*)decOutputBuf,
                max_output_samples, 0); // no fec
        
        if (outSamples != OPUS_BAD_ARG
                && outSamples != OPUS_BUFFER_TOO_SMALL
                && outSamples != OPUS_INTERNAL_ERROR
                && outSamples != OPUS_UNIMPLEMENTED
                && outSamples != OPUS_INVALID_STATE
                && outSamples != OPUS_ALLOC_FAIL)
